...
- [x] Phase 5 — skill packaging, demo, PyPI release
  ...
  - Fixed #3 (2026-07-12): `linker.py`'s `_scan_file_signals` now strips a trailing inline (`-- `/`# `) comment via new helper `_strip_inline_comment`, using a quote-balance heuristic (first marker occurrence preceded by an even count of both `'` and `"`) rather than a real tokenizer — ambiguous marker positions leave the line untouched, per ground rule 6 (precision over recall). MAPPING.md L8 updated first (ground rule 1) with the revised rule and rationale, plus a decision-log entry. TDD: two new focused unit tests on `_scan_file_signals` shown failing before the fix, one more asserting a `--` inside a string literal is correctly *not* treated as a comment start. Also edited the real fixture `tests/fixtures/tiny_repo/analytics/stg_orders.sql` to append a trailing comment to its existing `FROM orders` line, exercising the fix through the full `_tiny_result()` integration path with no change to expected edges. All good-first-issues from the post-release batch (#3, #4, #5, #6) are now closed; only issue-tracker/community follow-up remains as open-ended future work.
