...
`source_file` (nullable — see surprise 4). Capped at 3 per extraction chunk
by convention, not enforced by the schema.
...
4. **Hyperedge `source_file` can be `null`.** Discovered running the Phase 5c
   demo against a real `graphify .` extraction of `dbt-labs/jaffle-shop`
   (`tiny_graph.json`'s two hyperedges both happened to have a single
   attributable file, so Phase 0 modeled the field as required). When a
   hyperedge groups nodes with no single natural originating file (e.g. a
   "Project Configuration and Dependencies" hyperedge spanning
   `dbt_project.yml`/`packages.yml`/`requirements.txt`/`Taskfile.yml`),
   graphify emits `source_file: null`. `schema.py`'s `Hyperedge.source_file`
   is `str | None`; nothing downstream (exporter/importer/linker) reads
   hyperedge `source_file`, so this is a pure permissiveness fix, no
   behavior change.
5. **Some AST `code` nodes carry `source_file: ""`.** Also found running
   real `graphify .` over jaffle-shop: an `AuthHeader` class nested inside
   `.github/workflows/scripts/dbt_cloud_run_job.py` was extracted with
   `source_file: ""` (`source_location: ""` too) instead of its containing
   file's path — graphify couldn't attribute it, unlike the sibling
   top-level nodes in the same file which all resolved correctly. `""` is
   not a "missing" sentinel Python treats specially: `Path("").read_text()`
   resolves to the cwd and raises `IsADirectoryError`, not `FileNotFoundError`.
   `linker.py`'s `_ast_nodes_by_file` (L7) now excludes nodes with a falsy
   `source_file` before grouping-by-file for on-disk signal scanning — such
   a node can never be resolved to real source text, so it can never
   contribute a linker signal regardless.
...
| L6 | **Repo root**: `linker.link()` takes an explicit `repo_root: Path` (CLI: `--repo-root`, default `.`), used to discover source files absent from `graph.json` (`*.sql`, per §1 surprise 1 — graphify's structural pass is Python-only) *and*, as a fallback, to resolve existing `_origin: ast` node `source_file` values. **Revised 2026-07-11** (Phase 5c end-to-end run against real `dbt-labs/jaffle-shop` output): the original decision assumed a relative `source_file` always resolves as-is because graphify's CWD at extraction time and the linker's CWD at link time are the same process/directory. That assumption broke on first real use — `graphify .` is conventionally run with CWD *inside* the cloned repo, while `okf-bridge link` is invoked later from wherever (a demo Makefile, CI, another tool), by which point CWD no longer matches. Fix (`_resolve_ast_source_file`): try `source_file` as-is first (absolute, or exists relative to the linker's own CWD — covers the original assumption, still true whenever nothing has changed CWD in between, e.g. the Phase 0/4 fixture tests); if that path doesn't exist, resolve as `repo_root / source_file` instead (covers the `cd repo && graphify .` convention, where `repo_root` is passed as that same directory). `repo_root` remains an explicit, given input rather than inferred from the graph, for the reason originally stated: `.sql` files are invisible to `graph.json` by construction, so nothing about existing nodes can locate a sibling directory the graph never saw. | MUST |
| L7 | **Scan granularity is per-file, not per-node.** For every `file_type: code` / `_origin: ast` node's `source_file` (deduplicated), and for every `*.sql` file found under `repo_root`, the whole file text is scanned once. Rationale: `source_location` is a single line pointer, not a span, so it cannot bound "the text belonging to this node" for a multi-line SQL string; scanning the file once and dedup­ing matches is simpler and avoids fabricating span-inference logic. The **edge source** for a `.py` file is that file's *module node* (the node with the lowest line number among its `_origin: ast` nodes — ties broken by node id); for a `.sql` file it is a synthetic node `sql:<path relative to repo_root>`, `file_type: code`, emitted alongside the edges so the output graph stays self-consistent — **unless** an existing `file_type: code` graph node's label unambiguously stem-matches the file's stem (L7a), in which case that node's id is reused as the edge source instead and no synthetic node is emitted for it. Semantically-extracted nodes (no `_origin: "ast"`, e.g. `app_service_revenue_query` in the tiny fixture — see §1 surprise 4) are excluded from *file/module inference* (they never anchor a `.py` module-node lookup): their `source_file` is relative to the *document* they were extracted from, not the code tree, and using it would misattribute edges. L7a's reuse check is a different, narrower thing — it matches by *label*, never by `source_file`, so it stays safe even though semantically-extracted nodes are exactly the case it exists to catch (see L7a). | MUST |
| L7a | **Sql-file/native-node identity reuse (added 2026-07-11, Phase 5c end-to-end run against real `dbt-labs/jaffle-shop` output).** The original L7 assumption — a `.sql` file always produces zero graph nodes, so its synthetic `sql:<path>` node is always a fresh identity — broke on first real use: graphify's semantic (LLM) pass named several jaffle-shop dbt models (e.g. "stg_orders model") from their companion `schema.yml` files even though the `.sql` files themselves produced no AST nodes (no `tree_sitter_sql`). Left unhandled, this produces *two* disconnected graph identities for one real-world dbt model: the semantic node (reachable via L10's exact-name fallback) and the linker's own synthetic `sql:<path>` node (reachable only if *that file's own text* happens to reference an already-catalogued table — a staging model whose only reference is an uncatalogued raw source table gets a synthetic node with **zero** edges, an orphan masquerading as "the file"). Fix: before minting a `.sql` file's synthetic node, singularize its stem (same normalization as L10) and look for existing `file_type: code` graph nodes whose label token set contains that stem. Exactly one distinct matching label → reuse that node's id as the edge source, skip the synthetic node entirely, for every edge signal found in the `.sql` file's own text (`_scan_file_signals` output is unaffected — only the *source id* changes). Zero or ambiguous (2+ distinct labels) matches → fall back to the original L7 synthetic-node behavior, unchanged. This reuses L10's exact-name mechanism and trust level (a name-token match, not a `source_file` match — semantically-extracted nodes' `source_file` remains untrustworthy for file attribution per §1 surprise 4, so this never violates that rule) but for a different purpose: L10 links a *known* node to a *table concept*; L7a decides which node id a *newly-scanned `.sql` file* should attach its edges to, so the file's own signals land on the identity that's already connected to the rest of the graph instead of a disconnected duplicate. | MUST |
| L8 | **Comment stripping**: a whole line is dropped before scanning if its lstripped text starts with `#` (Python) or `--` (SQL). **Revised 2026-07-12** (issue #3): a trailing inline (end-of-line) comment is now also stripped from an otherwise-kept line, but *only* when the comment marker's position is unambiguously outside a string literal — computed as: scan the line's occurrences of the marker left to right, and take the first one preceded by an even number of both `'` and `"` characters (i.e. no quote left open before it). If no occurrence qualifies (every marker position sits after an odd/unmatched quote), the line is left completely unmodified — no truncation, no partial guess. This is a quote-balance heuristic, not a real tokenizer (it does not know about escaped quotes, nested quoting, or dialect-specific string syntax); it deliberately errs toward under-stripping (a missed inline comment can produce no more than what v1 already tolerated) rather than over-stripping (which could truncate a legitimate match hiding after a false comment marker inside a string — the wrong failure mode per ground rule 6, precision over recall). | MUST |
...
| L11 | **Linker edge target id** is `okf:<table-concept-id>` — the same id an `okf-bridge import` of the same bundle would assign that concept (Import rule I1). | MUST |
...
| L13 | **Linker output is self-contained: synthetic nodes on *both* sides of every edge.** `link()` already emitted a synthetic `sql:<path>` code-side node for `.sql` files (L7, since those never appear in `graph.json`); it now *also* emits a synthetic `file_type: concept` node for every OKF table concept an edge actually targets — `id` = the L11 `okf:<concept-id>` (so it's identical to what `okf-bridge import` would produce), `label` = the concept's `title` (falling back to its `concept_id` if untitled), `source_file` = the concept's `.md` path under the bundle root. **Revised 2026-07-11**, replacing the original L11 composability claim ("so `link` output composes with `import` + `merge-graphs` without inventing a second identity"): that claim doesn't hold against `graphify merge-graphs`'s real behavior, discovered running the Phase 5c demo end-to-end. `merge-graphs` namespaces every node id by its source *file*, specifically to prevent accidental collisions across repos (its own diagnostic: `repo dir names collide; using distinct tags`) — so the *same* string id `okf:tables/customers` coming from two separately-merged files (an `okf-bridge import` of the bundle, and a separate `okf-bridge link` output referencing the same id) never collapses into one node; they land as two distinct, differently-tagged nodes, and `graphify path`/`explain` see a real, dangling, label-less placeholder for whichever copy has no matching node in its *own* file. Emitting the target node directly in `link()`'s own output sidesteps cross-file id reuse entirely: a single `link()` output file is internally consistent (source + target nodes + edges together), and `graphify merge-graphs` no longer needs to be trusted to unify anything across files for the linker's own edges to resolve. | MUST |
...
| 2026-07-11 | §Linking L7a added: a `.sql` file's synthetic `sql:<path>` node is now skipped in favor of an existing `file_type: code` node when its label unambiguously stem-matches the file's stem. | Running the Phase 5c demo end-to-end against real `dbt-labs/jaffle-shop` output surfaced a graph where `graphify path "stg_orders.sql" "customers"` found no path: graphify's semantic pass had already named the `stg_orders` model (from its `schema.yml`) as a graph node reachable via L10, while `okf-bridge link` unconditionally minted a *second*, disconnected `sql:models/staging/stg_orders.sql` node for the same file — orphaned because that file's own SQL text only references an uncatalogued raw source table, so no signal-based edge ever attached to it. The tiny fixture's `.sql` files have no same-stem native-node twin, so L7a is inert there and the Phase 4 acceptance tests are unaffected. |
| 2026-07-12 | §Linking L8 revised: inline (end-of-line) `-- `/`# ` comments are now stripped when a quote-balance check shows the marker sits outside a string literal. | Filed as good-first-issue #3: `SELECT * FROM orders  -- staging table, see model docs` is a realistic pattern the original whole-line-only rule missed entirely. A full tokenizer was rejected as speculative parsing (ground rule against unvalidated speculative parsing); the quote-balance heuristic is conservative by construction — ambiguous marker positions leave the line untouched rather than guessing, keeping the linker's precision-over-recall posture (ground rule 6). |
